...
class Index:
    ...
    def _initialize_db(self):
        ...
        with sqlite3.connect(config.index_path, detect_types=sqlite3.PARSE_DECLTYPES) as conn:
            cursor = conn.cursor()
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_embeddings_source_id ON embeddings (source_id)')
            conn.commit()
    ...
    def load(self):
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute('SELECT id, source_id, embedding, section_from, section_to FROM embeddings')
            self.embeddings = []
            for row in cursor.fetchall():
        ...
        self.logger.info(f"Loaded {len(self.sources)} sources and {len(self.embeddings)} embeddings from the database")
